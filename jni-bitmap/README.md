...
[Link](https://blog.csdn.net/chzphoenix/article/details/42171653)

## Step

1. 用 AndroidBitmap_getInfo() 函数从位图句柄（从JNI得到）获得信息（宽度、高度、像素格式）
2. 用 AndroidBitmap_lockPixels() 对像素缓存上锁，即获得该缓存的指针。
3. 用C/C++ 对这个缓冲区进行读写
4. 用 AndroidBitmap_unlockPixels() 解锁
